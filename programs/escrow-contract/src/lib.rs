...
declare_id!("Dw9sRvAweUmzqoBxzNFBSGcQsWAZB4xyuWXWxMXnqqMJ");
...
impl<'info> Exchange<'info> {
    fn transfer_to_initializer_context(&self) -> CpiContext<'_, '_, '_, 'info, Transfer<'info>> {
        let cpi_accounts = Transfer {
    ...
    fn transfer_nft_to_taker_context(&self) -> CpiContext<'_, '_, '_, 'info, Transfer<'info>> {
        let cpi_accounts = Transfer {
            from: self.vault_account.to_account_info().clone(),
